...
def rag_cosine_similarity(vec1, vec2):
    try:
...
async def rag_get_embedding(text, max_retries=3):
    if not API_KEY:
...
async def rag_find_similar_content(query_embedding, conn):
    try:
        ...
        for chunk in discourse_chunks:
            try:
                ...
                similarity = rag_cosine_similarity(query_embedding, embedding)
...
async def rag_enrich_with_adjacent_chunks(conn, results):
    try:
...
async def rag_generate_answer(question, relevant_results, max_retries=2):
    if not API_KEY:
        error_msg = "API_KEY environment variable not set"
        logger.error(error_msg)
        raise HTTPException(status_code=500, detail=error_msg)
...
async def rag_process_multimodal_query(question, image_base64):
    if not API_KEY:
        error_msg = "API_KEY environment variable not set"
        logger.error(error_msg)
        raise HTTPException(status_code=500, detail=error_msg)
    ...
    try:
        ...
        if not image_base64:
            ...
            return await rag_get_embedding(question)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    return await rag_get_embedding(combined_query)
                else:
                    error_text = await response.text()
...
def rag_parse_llm_response(response):
    try:
...
# Define API routes
@app.post("/api/")
async def rag_query_knowledge_base(request: QueryRequest):
    try:
        ...
        try:
            ...
            relevant_results = await rag_find_similar_content(query_embedding, conn)
            ...
            enriched_results = await rag_enrich_with_adjacent_chunks(conn, relevant_results)
            
            # Generate answer
            logger.info("Generating answer")
            llm_response = await rag_generate_answer(request.question, enriched_results)
            
            # Parse the response
            logger.info("Parsing LLM response")
            result = rag_parse_llm_response(llm_response)
